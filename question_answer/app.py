...
def index():
    ...
    questions_cur = db.execute('''select questions.id as question_id, 
                                  ...
                                  from questions join users as askers on askers.id = questions.ask_by_id 
...
def ask():
    ...
    if request.method == 'POST':
        db.execute('insert into questions (question_text, ask_by_id, expert_id) values (?, ?, ?)', [request.form['question'], user['id'], request.form['expert']])
        db.commit()
